...
From there checkout the `2015winter` branch, then create and checkout ai branch named
`2015winter-student`
...
We will use the class project hosted on github.
The url is https://github.com/mikeizbicki/ucr-cs100.git 

###I think the stream is flowing away from me

Since our repository was cloned it from the URL above we can now push the changes to github using 'git push'.
This uploads our version of the repository to github for collaboration.
    
    $ git push <url> <branch>
    $ git push https://github.com/mikeizbicki/ucr-cs100.git2015winter-student

Isn't there a better way to push changes to a URL you ask. Git has what is called a remote
to handle this issue.

####I found another stream attached to this one

First to pull from a location that is not in your forked repo you will have to add a remote.
...
Git uses remotes to allow us to perform many actions.
...
We might want to do this when we have forked a remote repository that we do not have push access to.
...
Later in the guide an local upstream branch will be created, this how you may want to manage your remotes.
...
To alveiate the issue of typing the remote and the branch every time we can set up a tracking branch.
...
Now that you can create, and push changes to your own git repo its time to learn how grab external changes.
...
This information is stored in `FETCH_HEAD`.
...
So if you want your changes saved you will need to create and checkout a branch from this point, or any point before switching to another branch.
...
Some that will affect us later or else there is no need to create the local upstream branch.

    $ git checkout -b 2015winter-upstream

This is how we use fetch to create a remote branch in any repository.
...
If the merge finishes without any problems there will be a fast-forward. 
...
This starts a test editor to help deal with the changes.
...
Up to this point 'git remote' was used to add remotes to a repository, 'git fetch' was used to grab those repos, 'git merge' was used to merge those repos, 'git meregtool' was used to resolve the conflicts if any.
...
If results in the a merge conflict again we can use mergetool like we did last time.
It may not always be the case that we want to deal with merge conflicts.
Is there a way let would make it so pulling will only accept histories that are 'fast-forwards' to the branch.
To to this we pass the --ff-only flag to git pull
...
We can call this using `git rebase`. 
Git must use some point as base, its main refrence pointer. 
Once we have modifed our repository the pointer to our version is different than the upstream.
To make sure all of your changes happen after the changes of the remote repository we reestablish a base for our branch.
...
Just because rebasing a branch places your commit on top of the new base, it does not mean merge conflicts will not occur.
Instead when `git rebase` encounters a merge conflict when reapplying your commit, it will let you fix 'your' commit that is being applied.
...
Instead your change is the newest commit.
...
Not really this is the original author commit time, but not the rebase commit time.

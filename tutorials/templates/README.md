...
```
Templates are as complicated as you want to make them. If you want to use a template for a more sophisticated function, then the writing the template for the function might get a little messy.
...
```
First off, you may have noticed something different about this template. Here, `class` is used instead of `typename`. To keep things simple, there’s no real difference between these two declarations. Both can be used interchangeably. 
...
```
The second parameter is an allocator class that defines how memory will be allocated if the vector needs more memory. In most cases, you wouldn't need to worry about it because it is set by default. This is because it is a part of a **default template argument**.
...
Below is a normal class object that stores data of type int.
```
class Object
{
    int data;
...
To turn our class into a template, we simply replace all occurrences of int with the template name and add the template parameter at the top just like a you would for a function.
```
template <class type>
class Object
{
...
Well, you see... templates are Turing Complete. In this case, this means that they can be used to solve any problems with regards to typing whether it is storing memory or manipulating it. 

...
We can see that in the third line, Valgrind says that the memory leak is caused within the main function. In the second line, Valgrind further says that the leak is caused by the new[] operator. We can also see that all the memory that was not deallocated was definitely lost, meaning we cannot retrieve this memory.

#Topic: Dynamic and static linking

##1. Introduction Dynamic Link and Static Link:
* **Intro:**

    Linking is a process of collecting and combining multiple [object files](http://en.wikipedia.org/wiki/Object_file) into a single [executable file](http://en.wikipedia.org/wiki/Executable).
...
##2. Difference between Dynamic Link and Static Link:
* **Advantage of dynamic link:**
    1. Coding Sharing: Program can share identical code instead of owning individual copies of the same library; as a result, the size of the program is relatively smaller when using dynamic link.

    2. Automatic Updates: When a new version of a shared library is released, it automatically replaces its old version. In other words, when user runs a program, the program always automatically uses the newest version of the shared library. As a result, User does not need to recompile the program if the shared library gets updated.

* **Disadvantage of dynamic link:**
    1. Time cost: a run-time loading cost because all the names of the files in shared library need to be connected to the actual file at the run time.

    2. [DLL HELL](http://en.wikipedia.org/wiki/DLL_Hell): 
In Windows, suppose that there is a program A uses shared library (dll) V2.0. When you install a program B that requires the same library with V1.0, you may find program A stops working because the V2.0 dll has been overwritten with V1.0.
...
* **Advantage of static link:**
    1. Do not need to carry along a copy of the library that is being used.

    2. Time efficiency: There is no additional run-time loading time costs since the static library get complied into the final executable file at the compile time.

* **Disadvantage of static link:**
    1. The size of program is relatively bigger when the program uses static link instead of using dynamic link.

    2. Every time when the static library gets updated, the whole program needs to get recompiled in order to install the update.

    [For more information](http://stackoverflow.com/questions/140061/when-to-use-dynamic-vs-static-libraries)

##3. Examples of Using Dynamic Link and Static Link:
...
##4. Run both Static and Dynamic Linked Final Executable File

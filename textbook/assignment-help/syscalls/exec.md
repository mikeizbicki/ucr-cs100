...
This file will include all the system calls in order to use `exec`. 
It will include `fork`, `wait` and `exec` itself. 
`fork` is necessary when using `exec` because `exec` will end the program upon completion and only return if it failed. 
...
If you are creating your own bash shell you will HAVE to use `fork`. 
You will use the child process of the `fork` to execute all of your commands because `exec` will end the process its running in upon completion.
...
Here we’re setting `pid`, which is an `int`, to the return value of `fork`. 
...
You have to have the `exit()` statement when you’re done executing everything you want to in the child. If the exit statement isn't present, the child will continue executing through the rest of the code, which is likely intended for the parent. 
...
This prevents two processes from running at once. 
Not using `wait` may create errors because otherwise, the child may not necessarily end before the parent creating zombie or, in this specific case, orphaned processes. 
...
examples: This is an example of `wait` with `fork`.
```
int pid = fork();
...
The advantages of using `waitpid` or `waitid` over simply `wait`, is that wait will only stop the parent process until one of the children ends; it will not wait for any specific child or multiple ones. 
`waitpid` and `waitid` can wait for specific processes to end, either by its pid or its process group ID, and they can also wait for any child.
...
`waitid` does not take an `int *`, but instead a `siginfo_t` struct that will give even more information on the exit status of the child. These returned errors are useful for finding out whether or not your child process ended with an error, as, if it did, `wait` will still succeed even though you likely want to output some sort of message. 
However, you normally do not need the amount of information given to you by `waitid` to see whether this happened. `waitpid` and `waitid` also have the last parameter `int options`. 
...
Unless you are specifically required to use `execv` (e.g. your instructor wants you to use the path variable to find the particular command the user input), PLEASE use `execvp`. It will save you so much time, and will give you many more hours of much needed sleep.

We will demonstrate you an example of `execvp` that was used in the child function, in a program meant to create a bash shell:
...
In this example, `file` is `argv[0]` and `argv` is `argv[]`. 
`argv` is a char pointer pointer containing the user input of what commands they wish to execute.
...
As we pointed you above, `execvp` finds the path for you. 

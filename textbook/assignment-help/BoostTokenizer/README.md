...
The Boost tokenizer provides an easy way to parse through a string, using three `TokenizerFunc` models for different tasks.

The `char_separator` model creates tokens based on delimiters and allows you to choose dropped delimiters, kept delimiters, and whether to keep empty tokens.

The `escaped_list_separator` model creates tokens based on delimiters.
This model can use quote characters to make sure delimiters are not interpreted and can use escape characters for alternate interpretations of special characters.

Finally, the `offset_separator` model creates tokens based on a sequence of integer offsets.

The Boost Tokenizer package provides a variety of ways to make parsing much easier for almost any project.

For example, if you are making a program that reads and interprets user input commands and options separated by whitespace, you would want to filter out the whitespace and find each command or command option.
With either the `char_separator` model or the `escaped_list_separator` model, all you have to do is add whitespace to the delimiter set.

What if you want certain delimiters to result in a different behavior, such as having a certain character connect two commands?
In this case, using the `char_separator` model, you can just keep that delimiter by adding it the the set of kept delimiters and have the program react appropriately when finding a kept delimiter.
...
Using the `escaped_list_separator`, all you have to do is add the `"` character to the quote character set.

...
In CS 100, professor requires us to sort our file display alphabetacially by "ls" function. 
Instead of writing it ourselves, we could use the C++ Standard Library `sort` with less codes and higher efficiency. 
Besides, we will discuss the char array pointer memory allocation.
...
In fact, `sort` goes over all successive arrarys from the start address to the end address. 
...
Fistly, we should look at compare function. 
...
```
This needs c++11 standard to compile. 
...
In some sence, origin of char array seems like a container. 
...
Then, you will need to allocate some memory space to store the message and free it when you finish. 
Let's review the 10th example to compare:
```
bool compare(const char *p1,const char *p2)//sort alphabetically
{
	...
...
```
It looks fine. 
Also, we may have the same result if we run it on the same char arrays:
```
abc b bac dfp DFq ktw pab slm Z
```

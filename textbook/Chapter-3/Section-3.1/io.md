...
The way `dup2` works is it takes two parameters `int oldfd` and `int newfd`.
`oldfd` is the file descriptor you want to change to `newfd` by copying `oldfd` to `newfd`.

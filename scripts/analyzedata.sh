...
function timelastcommit {
    ...
    git log --pretty=format:'%at' | head -n1
    cd ../..
}
...
function timefirstcommit {
    ...
    git log --pretty=format:'%at' | awk "\$1 > $starttime {print}" | tail -n1
    cd ../..
}
...
function totaltime {
    ...
    #echo "starttime=$starttime; stoptime=$stoptime" >&2
    if [ -z "$starttime" ] || [ -z "$stoptime" ]; then
        echo 0
    else
...
function linesOfCode {
    ...
    git checkout "$2" --quiet 2>/dev/null
    if [ -d src ]; then
        wc -l src/* 2>/dev/null | tail -n1 | grep -o '^[[:space:]]*[[:digit:]]*[[:space:]]'
    else
        echo 0
    fi
    cd ../..
...
#echo "set xtics 86400; set grid xtics" >> scratch/out.gnu
    ...
    unset key
    unset ytics
    set arrow 1 from $date,0 to $date,160 nohead
    set xtics nomirror
    set xtics ($xtics)
...
assn2duedate="1416556799"
#prevduedate=$assn2duedate-86400*6
prevduedate=$assn1duedate+86400
duedate=$assn2duedate

#foreachStudent "timefirstcommit" ""
#exit
...
foreachStudent "timefirstcommit" "$prevduedate"> $datafile.timefirstcommit
#foreachStudent "timefirstcommit" "$tag" "1415011199"> $datafile.timefirstcommit
foreachStudent "totaltime" "$prevduedate" "$tag" > $datafile.totaltime
